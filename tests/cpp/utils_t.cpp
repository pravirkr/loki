...
TEST_CASE("find_neighbouring_indices edge cases",
    ...
    SECTION("Empty indices") {
        ...
        REQUIRE_THROWS_AS(loki::find_neighbouring_indices(empty_indices, 0, 1),
                          std::invalid_argument);
    }

    SECTION("Num is zero") {
        std::vector<size_t> indices = {0, 2, 4, 6, 8};
        REQUIRE_THROWS_AS(loki::find_neighbouring_indices(indices, 4, 0),
                          std::invalid_argument);
    }

    SECTION("Single element indices") {
        std::vector<size_t> single_index = {5};
        auto result = loki::find_neighbouring_indices(single_index, 5, 3);
