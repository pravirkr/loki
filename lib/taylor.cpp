...
void poly_shift_add_batch(const xt::xtensor<float, 3>& segment_batch,
    ...
    const SizeType n_batch = segment_batch.shape(0);
    const SizeType n_comps = segment_batch.shape(1);
    const SizeType n_cols  = segment_batch.shape(2); // nbins

    // Stack-allocated buffer for temporary rotated data (reuse across batches)
    std::vector<float> temp_buffer(2 * n_cols);
    float* __restrict__ temp_ptr = temp_buffer.data();

    for (SizeType irow = 0; irow < n_batch; ++irow) {
        const SizeType shift_raw = shift_batch[irow];
        const SizeType shift     = (shift_raw % n_cols + n_cols) % n_cols;
        const SizeType fold_idx  = isuggest_batch[irow];

        // Get restrict pointers for current batch item
        const float* __restrict__ segment_e =
            segment_batch.data() + ((irow * n_comps + 0) * n_cols);
        const float* __restrict__ segment_v =
            segment_batch.data() + ((irow * n_comps + 1) * n_cols);
        const float* __restrict__ fold_e =
            folds.data() + ((fold_idx * n_comps + 0) * n_cols);
        const float* __restrict__ fold_v =
            folds.data() + ((fold_idx * n_comps + 1) * n_cols);
        float* __restrict__ out_e =
            out.data() + ((irow * n_comps + 0) * n_cols);
        float* __restrict__ out_v =
            out.data() + ((irow * n_comps + 1) * n_cols);

        // Handle zero shift case (common optimization)
        if (shift == 0) {
            // Direct addition without rotation
            for (SizeType j = 0; j < n_cols; ++j) {
                out_e[j] = fold_e[j] + segment_e[j];
                out_v[j] = fold_v[j] + segment_v[j];
            }
        } else {
            // Optimized circular shift: rotate segment into temp buffer
            // For shift left by 'shift', we copy from position 'shift' onwards
            // first
            const SizeType copy1_size = n_cols - shift;
            const SizeType copy2_size = shift;

            // Rotate E channel
            std::copy_n(segment_e + shift, copy1_size, temp_ptr);
            std::copy_n(segment_e, copy2_size, temp_ptr + copy1_size);

            // Rotate V channel
            std::copy_n(segment_v + shift, copy1_size, temp_ptr + n_cols);
            std::copy_n(segment_v, copy2_size, temp_ptr + n_cols + copy1_size);

            // Add rotated segment to fold in single vectorizable loop
            const SizeType total_size               = 2 * n_cols;
            const float* __restrict__ fold_combined = fold_e;
            float* __restrict__ out_combined        = out_e;

            for (SizeType j = 0; j < total_size; ++j) {
                out_combined[j] = fold_combined[j] + temp_ptr[j];
            }
        }
    }
}
...
void poly_shift_add_complex_batch(
    ...
    const SizeType n_batch   = segment_batch.shape(0);
    const SizeType n_comps   = segment_batch.shape(1);
    const SizeType nbins_f   = segment_batch.shape(2); // Frequency bins
    const SizeType fold_bins = (nbins_f - 1) * 2;      // FFT size

    // Constants for vectorization
    constexpr SizeType kUnrollFactor = 4;

    for (SizeType irow = 0; irow < n_batch; ++irow) {
        const auto shift    = static_cast<double>(shift_batch[irow]);
        const auto fold_idx = isuggest_batch[irow];

        // Calculate phase step per frequency bin
        const double phase_step_angle =
            -2.0 * std::numbers::pi * shift / static_cast<double>(fold_bins);
        ...
        const ComplexType* __restrict__ segment_e =
            segment_batch.data() + ((irow * n_comps + 0) * nbins_f);
        const ComplexType* __restrict__ segment_v =
            segment_batch.data() + ((irow * n_comps + 1) * nbins_f);
        const ComplexType* __restrict__ fold_e =
            folds.data() + ((fold_idx * n_comps + 0) * nbins_f);
        const ComplexType* __restrict__ fold_v =
            folds.data() + ((fold_idx * n_comps + 1) * nbins_f);
        ComplexType* __restrict__ out_e =
            out.data() + ((irow * n_comps + 0) * nbins_f);
        ComplexType* __restrict__ out_v =
            out.data() + ((irow * n_comps + 1) * nbins_f);
