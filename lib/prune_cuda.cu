...
public:
    ...
    EPMultiPassCUDATypedImpl& operator=(EPMultiPassCUDATypedImpl&&) = delete;
    ...
    Impl(plans::FFAPlan<HostFoldType> ffa_plan,
        ...
        if constexpr (std::is_same_v<FoldTypeCUDA, ComplexTypeCUDA>) {
            m_world_tree = std::make_unique<utils::WorldTreeCUDA<FoldTypeCUDA>>(
                ...
                max_batch_size);
            m_ep_workspace =
                std::make_unique<utils::EPWorkspaceCUDA<FoldTypeCUDA>>(
...
private:
    ...
    std::unique_ptr<utils::EPWorkspaceCUDA<FoldTypeCUDA>> m_ep_workspace;
    ...
    void execute_iteration_batched(cuda::std::span<const FoldTypeCUDA> ffa_fold,
        ...
        auto& scratch_ws = m_ep_workspace->scratch;
        ...
        while (total_processed < n_branches) {
            ...
            const auto n_leaves_batch = m_prune_funcs->branch(
                ...
                coord_prev, current_batch_size, branch_ws, scratch_ws, stream);
...
EPMultiPassCUDA::EPMultiPassCUDA(search::PulsarSearchConfig cfg,
                                 std::span<const float> threshold_scheme,
                                 std::optional<SizeType> n_runs,
                                 std::optional<std::vector<SizeType>> ref_segs,
                                 SizeType max_sugg,
...
EPMultiPassCUDA::EPMultiPassCUDA(EPMultiPassCUDA&& other) noexcept = default;
EPMultiPassCUDA&
