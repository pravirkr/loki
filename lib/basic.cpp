...
loki::ffa_resolve(std::span<const float> pset_cur,
    ...
    const auto t_ref_prev = (static_cast<float>(latter) - 0.5F) *
                            std::pow(2.0F, static_cast<float>(ffa_level - 1)) *
                            tseg_brute;
    std::vector<float> pset_prev(nparams, 0.0F);
    float delay_rel = 0.0F;

    if (nparams == 1) {
        pset_prev[0] = pset_cur[0];
    } else {
        std::vector<float> dvec_cur(nparams + 1, 0.0F);
        std::copy(pset_cur.begin(), pset_cur.end() - 1, dvec_cur.begin());
        std::vector<float> dvec_prev =
            loki::utils::shift_params(dvec_cur, static_cast<float>(t_ref_prev));
