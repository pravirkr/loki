...
ffa_resolve(std::span<const double> pset_cur,
    ...
    std::vector<double> pset_prev(nparams);
    double delay_rel;
    ...
    if (nparams == 1) {
        ...
        pset_prev.back() =
            pset_cur.back() * (1.0 + kvec_prev[nparams - 1] / utils::c_val);
        delay_rel = kvec_prev.back() / utils::c_val;
    }

    double phase_rel = kernels::get_phase_idx_helper(
        t_ref_prev, pset_prev.back(), nbins, delay_rel);

    std::vector<int> pindex_prev(nparams);
    for (int ip = 0; ip < nparams; ++ip) {
        pindex_prev[ip] =
            kernels::find_nearest_sorted_idx(parr_prev[ip], pset_prev[ip]);
    }
...
void loki::ffa_init(std::span<const float> ts,
                    ...
                    float tsegment_cur,
                    size_t nbins) {
    fold_brute_start(ts, std::span<const float> freq_arr,
                     std::span<float> fold, size_t chunk_len, size_t nbins,
                     float dt, float t_ref)
}
